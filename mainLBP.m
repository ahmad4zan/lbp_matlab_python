...
imageFolder = 'malimg_dataset/test';
...
for i = 1:numImages
 ...
 %% Visualize after LBP extraction
% % Plot the first two dimensions of the LBP feature vectors
% figure;
% gscatter(featureMatrix(:, 1), featureMatrix(:, 2), labels);
% xlabel('LBP Feature 1');
% ylabel('LBP Feature 2');
...
GMModels = cell(1, numGMMs);
numSamplesPerGMM = size(trainFeatures, 1) / numGMMs;
...
gm = fitgmdist(data,2,'Options',options);
...
for idx = 1:numGMMs
    ...
    hold on
    h = fcontour(gmPDF,[0 dMax]);
    % h= fcontour(gmPDF, [0 dMax], 'LineColor', rand(1,3), 'HandleVisibility', 'off');
...
% accuracies = zeros(1, numGMMs);
% 
% % Loop through each GMM model
% for gmmIdx = 1:numGMMs
% 
% %  predictedLabels = strings(numTestImages, 1);
% % 
