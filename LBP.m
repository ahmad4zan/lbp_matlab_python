...
%  Examples
%  --------
%       I=imread('rice.png'); I=imread('00002_940128_fa.ppm');
...
for i = 1:neighbors
  % set x y points for position P
  y = spoints(i,1)+origy;
  x = spoints(i,2)+origx;
  % Calculate floors, ceils and rounds for the x and y.
  fy = floor(y); cy = ceil(y); ry = round(y);
  fx = floor(x); cx = ceil(x); rx = round(x);
  % Check if interpolation is needed. sampling points that do not fall
  % within the pixels are interpolated using bilinear interpolation   
  if (abs(x - rx) < 1e-6) && (abs(y - ry) < 1e-6)
    % Interpolation is not needed, use original datatypes
    N = image(ry:ry+dy,rx:rx+dx);
    D = N >= C; % key of LBP find neighbour grater than central!
%     D = N ~= C; %xor
  else
...
w3*d_image(cy:cy+dy,fx:fx+dx) + w4*d_image(cy:cy+dy,cx:cx+dx);
    N = roundn(N,-4);
    D = N >= d_C; % key of LBP find neighbour grater than central!
%     D = N ~= d_C; %xor
  end  
...
% error(nargchk(2, 2, nargin, 'struct')) %ori
narginchk(2, 2) %fix
